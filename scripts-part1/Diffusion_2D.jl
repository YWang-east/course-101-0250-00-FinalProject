...
macro qx(ix,iy)  esc(:( -D_dx*(Hτ[$ix+1,$iy+1] - Hτ[$ix  ,$iy+1]) )) end
macro qy(ix,iy)  esc(:( -D_dy*(Hτ[$ix+1,$iy+1] - Hτ[$ix+1,$iy  ]) )) end
...
@views function diffusion_2D(; do_visu=false)
    ...
    R_H = @zeros(nx-2, ny-2) # residual without halo
    R_v = zeros(nx_v, ny_v)  # global residual
    ...
    for it = 1:nt 
        ...
        while R_m > tol
            ...
            iter += 1
            # if iter % 10 == 0
            #     println("time = $(round(it*dt, sigdigits=3)), iterations = $(iter), residual = $(R_m)")
            # end
        end
        # update H 
        H[:,:] = Hτ; niter += 1
        ...
        if do_visu && (it % nout == 0)
            ...
            if (me==0)
                ...
                display(heatmap(Xi_g, Yi_g, Array(H_v)'; opts...))
            end
        end
    end
